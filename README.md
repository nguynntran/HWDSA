...
class CSVGraphApp:
    def __init__(self, root):
        ...
        self.file_paths = []
        self.data_frames = []
        self.figure_config = {"width": 8, "height": 6, "dpi": 100}  # Default values
        ...
        self.configure_button = tk.Button(root, text="Configure Figure", command=self.configure_figure)
    ...
    def init_figure(self):
        """Initialize the figure with the current configuration."""
        self.figure = plt.Figure(figsize=(self.figure_config["width"], self.figure_config["height"]),
                                 dpi=self.figure_config["dpi"])
        self.canvas = FigureCanvasTkAgg(self.figure, self.root)
        self.canvas.get_tk_widget().pack()

    def configure_figure(self):
        """Open a dialog to configure figure settings."""
        width = simpledialog.askfloat("Figure Width", "Enter figure width (in inches):", initialvalue=self.figure_config["width"])
        height = simpledialog.askfloat("Figure Height", "Enter figure height (in inches):", initialvalue=self.figure_config["height"])
        dpi = simpledialog.askinteger("Figure DPI", "Enter figure DPI:", initialvalue=self.figure_config["dpi"])

        if width and height and dpi:
            self.figure_config.update({"width": width, "height": height, "dpi": dpi})
            self.init_figure()  # Reinitialize figure with new settings
            messagebox.showinfo("Figure Configuration", "Figure configuration updated successfully.")
